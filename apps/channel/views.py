...
from .serializers import (
    ...
    ChannelScheduledMessageSerializer,
)
...
class ChannelListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Channel.objects.filter(Q(owner=user) | Q(memberships__user=user))
            .distinct()
            .order_by("name")
        )
...
class ChannelMembershipUpdateDestroyView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = ChannelMembership.objects.filter(
            channel__id=channel_id, id=membership_id
        )
        return queryset
...
class CreateScheduledMessageView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(
            sender=self.request.user, channel=channel, scheduled_time=now().isoformat()
        )
