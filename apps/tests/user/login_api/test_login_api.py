...
def login_data(request, user_factory, fake_number):
    ...
    user = user_factory.create(phone_number=phone_number, is_active=True, is_verified=True)

    def valid_phone_number():
        return (
            200, {'phone_number': user.phone_number},
        )
    ...
    def invalid_phone_number():
        return (
            400, {'phone_number': 'invalid'},
        )

    def not_found_phone_number():
        return (
            404, {'phone_number': '+998911234567'},
        )
...
def test_login(login_data, api_client, mocker):
    ...
    mocker.patch('user.serializers.generate_otp', return_value=('otp_code', 'otp_secret'))
