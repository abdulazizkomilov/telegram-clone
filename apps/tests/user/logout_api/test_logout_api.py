...
def test_logout(mock_add_token, api_client, user_factory, mocker, tokens):
    ...
    mock_add_token.assert_any_call(
        uuid.UUID(str(user.id)), 'fake_token', "access", settings.SIMPLE_JWT["ACCESS_TOKEN_LIFETIME"]
    )
    mock_add_token.assert_any_call(
        uuid.UUID(str(user.id)), 'fake_token', "refresh", settings.SIMPLE_JWT["REFRESH_TOKEN_LIFETIME"]
    )
