...
class MockUserIndex:
    ...
    def search():
        ...
        mock_search_instance.query.return_value.execute.return_value = [
            MagicMock(to_dict=lambda: {'phone_number': '12345', 'first_name': 'John', 'last_name': 'Doe'})
        ]
...
class MockChannelIndex:
    ...
    def search():
        mock_search_instance = MagicMock()
        mock_search_instance.query.return_value.execute.return_value = [
            MagicMock(to_dict=lambda: {'name': 'General Channel', 'description': 'Channel for general discussions'})
        ]
        return mock_search_instance
...
def test_search_view(api_client, mocker, tokens, user_factory):
    ...
    expected_data = {
        ...
        'channels': [{'name': 'General Channel', 'description': 'Channel for general discussions'}],
    }
