...
class TestCreateScheduledMessageView:
    ...
    def test_successful_message_scheduling(self, setup_data, api_client, tokens, mocker):
        ...
        data = {
            ...
            "scheduled_time": (timezone.now() + timezone.timedelta(minutes=10)).isoformat()
        }

        response = client.post(f"/api/channels/{setup_data['channel'].id}/messages/schedule/", data, format="json")

        assert response.status_code == status.HTTP_201_CREATED
        assert ChannelScheduledMessage.objects.filter(channel=setup_data["channel"]).exists()
    ...
    def test_channel_not_found(self, setup_data, mocker, api_client, tokens):
        ...
        response = client.post(f"/api/channels/1/messages/schedule/", data, format="json")
