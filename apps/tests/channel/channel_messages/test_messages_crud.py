...
class TestChannelMessageListCreateView:
    ...
    def test_list_messages(self, mocker, tokens, api_client, channel, user_factory):
        ...
        ChannelMessage.objects.create(channel=channel, user=channel.owner, text="First Message")
        ChannelMessage.objects.create(channel=channel, user=member, text="Second Message")
        ...
        assert len(response.data['results']) == 2, "Expected two messages in the response"

    @patch('share.tasks.send_push_notification.delay')
    def test_create_message_as_owner(self, mock_send_push_notification, mocker, tokens, api_client, channel,
                                     user_factory):
        member = user_factory.create()
        ChannelMembership.objects.create(channel=channel, user=member)
        ...
        assert response.data["text"] == "Hello, this is a new message", "Text content mismatch"
    ...
    def test_create_message_as_non_owner(self, mocker, tokens, api_client, channel, user_factory):
        ...
        assert response.status_code == status.HTTP_403_FORBIDDEN, "Non-owner should not be able to create messages"
...
class TestChannelMessageDetailView:
    ...
    @pytest.fixture
    def message(self, channel, user_factory):
        user = user_factory.create()
        return ChannelMessage.objects.create(channel=channel, user=channel.owner, text="Test Message")
    ...
    def test_read_message(self, mocker, tokens, api_client, message, user_factory):
        ...
        response = client.get(f"/api/channels/{message.channel.id}/messages/{message.id}/")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_update_message_as_owner(self, mocker, tokens, api_client, message):
        ...
        response = client.patch(f"/api/channels/{message.channel.id}/messages/{message.id}/", data=data)

        assert response.status_code == status.HTTP_200_OK
        assert response.data["text"] == "Updated Message", "Text should be updated"

    def test_update_message_as_non_owner(self, mocker, tokens, api_client, message, user_factory):
        non_owner = user_factory.create()
        mock_redis_client = MagicMock()
        mocker.patch.object(TokenService, 'get_redis_client', return_value=mock_redis_client)
        ...
        assert response.status_code == status.HTTP_403_FORBIDDEN, "Non-owner should not be able to update messages"
