...
class TestGroupMessageCreateView:
    @pytest.fixture
    def setup(self, mocker, api_client, tokens, user_factory, group_factory, generate_test_image, generate_test_file):
        self.user = user_factory.create()
        self.group = group_factory.create()

        self.group_permission = GroupPermission.objects.create(group=self.group, can_send_media=True)
    ...
    def test_create_group_message_with_media(self, setup):
        ...
        response = self.client.post(f"/api/groups/{self.group.id}/messages/", data, format='multipart')
