...
class TestGroupAPI:
    ...
    @pytest.mark.parametrize("owner_type, expected_status", [
        ("owner", status.HTTP_200_OK),
        ("non_owner", status.HTTP_404_NOT_FOUND)
    ])
    def test_retrieve_group(self, mocker, api_client, tokens, user_factory, group_factory, owner_type, expected_status):
        owner = user_factory()
        member = user_factory()
    ...
    @pytest.mark.parametrize("is_owner, expected_status", [
        (True, status.HTTP_204_NO_CONTENT),
        (False, status.HTTP_403_FORBIDDEN)
    ])
    def test_delete_group(self, mocker, api_client, tokens, user_factory, group_factory, is_owner, expected_status):
        owner = user_factory()
        non_owner = user_factory()
        group = group_factory(name='Test Group', owner=owner)
        ...
        assert response.status_code == expected_status
        assert (not Group.objects.filter(id=group.id).exists()) if is_owner else Group.objects.filter(
            id=group.id).exists()
