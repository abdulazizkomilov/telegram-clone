...
class TestTokenAuthMiddleware:
    ...
    async def test_valid_token(self, mock_get_user, mock_jwt_decode, valid_token_payload, fake_scope):
        ...
        mock_get_user.return_value = User(id="123e4567-e89b-12d3-a456-426655440000", username="+998933333333")

        middleware = TokenAuthMiddleware(self.mock_asgi_app)
        await middleware(fake_scope, None, None)

        assert fake_scope["user"].id == "123e4567-e89b-12d3-a456-426655440000", \
    ...
    async def test_user_not_found(self, mock_get_user, mock_jwt_decode, valid_token_payload, fake_scope):
        ...
        assert isinstance(fake_scope["user"],
                          AnonymousUser), "Expected user to be AnonymousUser since user doesn't exist."
