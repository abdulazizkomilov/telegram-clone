...
class SearchView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        user_results = UserIndex.search().query("multi_match", query=query,
                                                fields=['phone_number', 'first_name', 'last_name']).execute()

        group_results = GroupIndex.search().query("match", name=query).execute()

        channel_results = ChannelIndex.search().query("multi_match", query=query,
